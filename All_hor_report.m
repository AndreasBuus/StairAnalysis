...
for sub = 1:length(names) % subjects
    ...
    for proto = proto_all % protocols 
        ...
        data{proto,ANG} = rescale(data{proto,ANG})*4+25; % rescale the signal; 
        posi = data{proto,ANG};
...
%% save before changes
gem_total_data = total_data; 

%% Task 0.2 Show average sweep for single subject
fprintf('script: TASK 0.2  .  .  .  .  .  .  .  .  .  .  .  .  .  .  .  .  '); tic

offset_ms([1,2,3,4]) = [15,10,00,00];  
offset_ms([5,6,7,8]) = [10,01,12,07];
offset_ms([11])      = [8];

change([1,2,3,4]) = [55,31,38,44]; 
change([5,6,7,8]) = [0 ,40, 0,48]; 
change([11])      = [54]; 

total_data = gem_total_data; 

offset_sample = ms2sec(offset_ms)*Fs; 
for sub = [1:numel(names(1:8)),11]
    data = total_data{1,1,sub};  
    type = total_type{:,:,sub}; yes = type{1}; no = type{2};
    for type = [SOL, TA, ANG, VEL]
        data{VER,type}(yes,:) = [zeros(numel(yes), offset_sample(sub)), data{VER,type}(yes,1:end-offset_sample(sub))];
    end
    total_data{1,1,sub} = data; 
end
...
subject = sub;            % Obtions: 1:8
proto = VER;            % Obtions: CTL, VER, HOR 
str_sen = ["Position", "Velocity", "Soleus", "Tibialis"];    % Obtions: "Soleus", "Tibialis","Position", "Velocity", "Acceleration"; 
show_FSR = false; 
...
if align_bool
    ...
    str_xlabel = "Time [ms]" + newline + "Align to step four"; 
else 
    % Load data 
    data = total_data{1,1,subject}; 
    x_axis = linspace(-4, 6-dt, N); 
    x_axis = sec2ms(x_axis); 
    str_xlabel = "Time [ms]" + newline + "Align to Force-Platform";
end 
...
fig = findobj('Name', 'Task 0.2');
if ~isempty(fig), close(fig); end
figure('Name','Task 0.2') ; 
    ...
    for i = 1:size(str_sen,2) % check and plot data included in str_sen
        ...
        switch proto
            case {VER, HOR} 
                ...
                if sensor_modality == ANG; plot(39,25,'.'); plot(59,25,'.');plot(80,25,'.'); end
                ...
                if sensor_modality == SOL 
                    
                    x1 = find(floor(x_axis) == 39); 
                    array1 = x1:x1+ms2sec(20)*Fs; 
                    x2 = find(floor(x_axis) == 59); 
                    array2 = x2:x2+ms2sec(20)*Fs; 
                    window_CTL(SLR,subject) = mean(mean(data{proto,sensor_modality}(no,array1),1)); 
                    window_SOL(SLR,subject) = mean(mean(data{proto,sensor_modality}(yes,array1),1)); 
                    window_CTL(MLR,subject) = mean(mean(data{proto,sensor_modality}(no,array2),1)); 
                    window_SOL(MLR,subject) = mean(mean(data{proto,sensor_modality}(yes,array2),1)); 
...
% step_index = total_step{1}; 
% 
% step_ms = step_index{CTL}(:,fall) - step_index{CTL}(:,rise);
% 
% floor(mean(step_ms))*dt
% 
% std(step_ms*dt) 


%% save before change
window_CTL_data = window_CTL; 
window_SOL_data = window_SOL; 
change_data = change;
%% VER fc 
window_SOL = window_SOL_data; 
window_CTL = window_CTL_data; 
change = change_data; 

% Remove zero indexes
exc_idx = [5,7]; 
window_CTL(:,exc_idx) = 0; % exclude 
window_SOL(:,exc_idx) = 0; % exclude
change(:,exc_idx)     = 0; % exclude 
zero_indices_CTL = any(window_CTL == 0,1); window_CTL(:,zero_indices_CTL) = []; 
zero_indices_SOL = any(window_SOL == 0,1); window_SOL(:,zero_indices_SOL) = []; 
zero_indices_CHA = any(change == 0,1);     change(:,zero_indices_CHA) = []; 
...
fig = findobj('Name', 'VER FC');
if ~isempty(fig), close(fig); end
figSize = [100 100 floor(width/3) floor(height/4)]; % where to plt and size
figure('Position',figSize,'Name','VER FC') ; hold on; xlim([0.5, 4.5])
title("Short Latency Window              Medium latency Window")
ylabel("Normalized"+newline+"Soleus activity")


for ref = [SLR, MLR]
    for sub = 1:size(window_SOL,2)
        if window_CTL(ref,sub) > window_SOL(ref,sub)
            plot([1+(ref-1)*2,2+(ref-1)*2], [window_CTL(ref,sub), window_SOL(ref,sub)], 'color', "blue","LineWidth",LineWidth)
        else 
            plot([1+(ref-1)*2,2+(ref-1)*2], [window_CTL(ref,sub), window_SOL(ref,sub)], 'color', "red", "LineWidth",LineWidth)
        end
    end
    plot(ones(size(window_CTL(ref,:)))+(ref-1)*2 , window_CTL(ref,:),'.', 'color', color_idv)
    plot(ones(size(window_SOL(ref,:)))+1+(ref-1)*2, window_SOL(ref,:),'.', 'color', color_idv)
end
...
legend(["Depressed","Facilitated","","","","","","","","","","","","","","","","","",""])

% subplot(212); hold on; ylim([0.5 2.5])
% xlabel("Onset [ms]"); 
% grid on; 
% for sub = 1:numel(window_SOL)
%     if window_CTL(sub) > window_SOL(sub)
%         plot(change(sub),1,'.', 'color', "blue")
%         plot(change(sub),1,'o', 'color', color_idv)
%     else
%         plot(change(sub),2,'.', 'color', "red")
%         plot(change(sub),2,'o', 'color', color_idv)
%     end 
% end 
% yticks([1 2]);
% yticklabels({'Depressed','Facilitated'});
...
if show_plot
    ...
    figSize = [50 50 width height]; % where to plt and size
    figure('Name', 'Pre-baseline','Position', figSize, 'unit', 'centimeters'); % begin plot 
    ...
    % Predictor 
        ...
        for i = 1:numel(sensor_type)
         ...
         %   if sensor_type(i) == ANG; title("Step " + k*2); end
            ...
            %y = smoothdata(y, row, 'gaussian', 100);
...
% ====================================
    ...
    [~, idx_y_ta]  = max(cellfun(@(x) diff(x), ylims(1:3)));
    [~, idx_y_sol] = max(cellfun(@(x) diff(x), ylims(4:6)));

    [~, idx_x_ta]  = max(cellfun(@(x) diff(x), xlims(1:3)));
    [~, idx_x_sol] = max(cellfun(@(x) diff(x), xlims(4:6)));
    idx_y_sol = idx_y_sol+3; 
    idx_x_sol = idx_x_sol+3; 
    for i = 1:numel(ax)
        if any(i == 1:3)
...
if pltShow 
    if exist("reg_data")
        height = 390/2; 
        width = 700;
        figSize = [300 250 width height]; % where to plt and size
        ...
        figure('name','slopes','Position', figSize, 'unit', 'centimeters'); % begin plot 
        hold on 
        ...
        for sensory_type = [SOL, TA]
            for step = 1:4
                ...
                elseif step == 4
                    legend(["", "Group mean", "Slope (subject)"])
                end
...
if show_plt
    ...
    for sub = inc_sub_hor
        ...
        SLR_begin = floor(ms2sec(offset(sub)+39)*Fs); 
        SLR_end   = floor(ms2sec(offset(sub)+59)*Fs); % [length=41]
        MLR_begin = floor(ms2sec(offset(sub)+60)*Fs); 
        MLR_end   = floor(ms2sec(offset(sub)+80)*Fs); % [length=41]
...
fig = findobj('Name', 'HOR new plot');
    ...
    subplot(211); hold on; title("Short-latency Window                    Medium-latency Window"); xlim([0.5 4.5])
        ...
        %Individuel SLR
        plot(ones(size(inc_sub_hor)),   x_sol_control(SLR,:), ".", "color", color_idv)
        plot(ones(size(inc_sub_hor))+1, x_sol_pert(SLR,:), ".", "color", color_idv)
        for i = 1:numel(inc_sub_hor)
            plot([1,2],[x_sol_control(SLR,i),x_sol_pert(SLR,i)], "color", color_idv)
        end

        %Individuel MLR
        plot(ones(size(inc_sub_hor))+2,   x_sol_control(MLR,:), ".", "color", color_idv)
        plot(ones(size(inc_sub_hor))+3, x_sol_pert(MLR,:), ".", "color", color_idv)
        for i = 1:numel(inc_sub_hor)
            plot([3,4],[x_sol_control(MLR,i),x_sol_pert(MLR,i)], "color", color_idv)
        end

        %Mean SLR
        plot(1, mean(x_sol_control(SLR,:)),"o", 'linewidth', 1,'color',color_mean )
        plot(2, mean(x_sol_pert(SLR,:)),"o", 'linewidth', 1,'color',color_mean)
        plot([1,2], [mean(x_sol_control(SLR,:)),mean(x_sol_pert(SLR,:))], 'linewidth',1.5, "color", color_mean) 

        %Mean MLR
        plot(3, mean(x_sol_control(MLR,:)),"o", 'linewidth', 1,'color',color_mean )
        plot(4, mean(x_sol_pert(MLR,:)),"o", 'linewidth', 1,'color',color_mean)
        plot([3, 4], [mean(x_sol_control(MLR,:)),mean(x_sol_pert(MLR,:))], 'linewidth',1.5, "color", color_mean) 

        plot([3.2,3.8], [2.5, 2.5],'-|', 'linewidth',1.5, "color", color_mean) 
        plot([3.5], [2.5],'*', 'linewidth',1.5, "color", "Black") 
    ...
    subplot(212); hold on; title("Short-latency Window                    Medium-latency Window"); xlim([0.5 4.5])
        ...
        %Individuel SLR
        plot(ones(size(inc_sub_hor)),   x_ta_control(SLR,:), ".", "color", color_idv)
        plot(ones(size(inc_sub_hor))+1, x_ta_pert(SLR,:), ".", "color", color_idv)
        for i = 1:numel(inc_sub_hor)
            plot([1,2],[x_ta_control(SLR,i),x_ta_pert(SLR,i)], "color", color_idv)
        end

        %Individuel MLR
        plot(ones(size(inc_sub_hor))+2,   x_ta_control(MLR,:), ".", "color", color_idv)
        plot(ones(size(inc_sub_hor))+3, x_ta_pert(MLR,:), ".", "color", color_idv)
        for i = 1:numel(inc_sub_hor)
            plot([3,4],[x_ta_control(MLR,i),x_ta_pert(MLR,i)], "color", color_idv)
        end

        %Mean SLR
        plot(1, mean(x_ta_control(SLR,:)),"o", 'linewidth', 1,'color',color_mean )
        plot(2, mean(x_ta_pert(SLR,:)),"o", 'linewidth', 1,'color',color_mean)
        plot([1,2], [mean(x_ta_control(SLR,:)),mean(x_ta_pert(SLR,:))], 'linewidth',1.5, "color", color_mean) 

        %Mean MLR
        plot(3, mean(x_ta_control(MLR,:)),"o", 'linewidth', 1,'color',color_mean )
        plot(4, mean(x_ta_pert(MLR,:)),"o", 'linewidth', 1,'color',color_mean)
        plot([3, 4], [mean(x_ta_control(MLR,:)),mean(x_ta_pert(MLR,:))], 'linewidth',1.5, "color", color_mean) 
