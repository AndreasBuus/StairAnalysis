...
% Example cell array
cell_2x2x2 = cell(2, 2, 2);
cell_2x2x2{1,1,1} = [1 2; 3 4];
cell_2x2x2{1,1,2} = [5 6; 7 8];
cell_2x2x2{1,2,1} = [9 10; 11 12];
cell_2x2x2{1,2,2} = [13 14; 15 16];
cell_2x2x2{2,1,1} = [17 18; 19 20];
cell_2x2x2{2,1,2} = [21 22; 23 24];
cell_2x2x2{2,2,1} = [25 26; 27 28];
cell_2x2x2{2,2,2} = [29 30; 31 32];
...
% Reshape to 2x2
kage = cell_2x2x2{:};
cell_2x2 = reshape(cell_2x2x2{1,:,:},[2,3]);
...
if normalize 
    ...
    for sub = 1:length(names) % loop through subjects  
        ...
        dim = size(data_all); 
        single_subject = reshape(data_all, [dim(2:3)]);
        % Load data 
        data = data_all{sub,:,:};
