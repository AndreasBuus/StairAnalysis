...
folderpath_data_part1 = main_folderpath + "/Data_MrKick/" + "nikolas_6marts.mat";
...
data{CTL,SOL} = [SOL_CTL1];       clear SOL_CTL1 SOL_CTL2; 
data{CTL,TA}  = [TA_CTL1];         clear TA_CTL1 TA_CTL2; 
data{CTL,ANG}  = [angle_CTL1];  clear angle_CTL1 angle_CTL2; 
data{CTL,FSR}  = [FSR_CTL1];      clear FSR_CTL1 FSR_CTL2; 

% Acquisition Set-Up
sweep_length = 12;              % Signal length in second
...
% Exclude sweep 
exclude_CTL = [2,3,14,39,40,41];               % excluded control sweeps
...
if correct_fall_edge 
    ...
    for sweep = 1:size(data{CTL, ANG},1)
        ...
        fall = [2,4,6]; 
        for i = 1:3
            distancefromval = locs - step_index{CTL}(sweep, rise(i));  
